...
- Продлёнка — только пн–пт (dayIdx <= 4): правило реализовано в renderDayLessons И в
  hasExtended-проверках renderAll (мобильная + десктопная ветки) — менять ТОЛЬКО синхронно
  во всех трёх местах и в обеих копиях script.js, иначе на сб/вс пропадут шутки
  (день ошибочно сочтут «занятым») или появятся пустые панели.
