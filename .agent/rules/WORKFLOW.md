...
- grep/read БЕЗ query graph.json — КРИТИЧЕСКИЙ ЗАПРЕТ (кроме явных исключений в п.13). Сначала graph.json, потом файлы.
...
└─ Русские строки брать из реальных файлов/кодовых точек, не набирать вручную (гомоглизы).
...
- Шутки выходных (SAT_MESSAGES / SUN_MESSAGES, renderWeekendMsg) — пользователь их ценит.
...
- Продлёнка — только пн–пт (dayIdx <= 4): правило реализовано в renderDayLessons, в
  hasExtended-проверках renderAll (мобильная + десктопная ветки) и в getTodayItems
  (только assets-копия: строка статуса и прогресс-бар) — менять ТОЛЬКО синхронно
  во всех местах и в обеих копиях script.js, иначе на сб/вс пропадут шутки,
  появятся пустые панели или «Сейчас: Полдник» в выходной.
- Сб/вс — «пустые» дни: показывают шутку дня (renderWeekendMsg + SAT_MESSAGES/SUN_MESSAGES).
...
11. ФОРМАТ ОТВЕТА
- Простая правка: фрагмент кода ±3 строки вокруг изменений → отчёт (п.10).
- Сложная задача: План → Реализация (полный файл или diff) → Проверка → отчёт (п.10).
...
- Дублирование общих файлов в корне и assets/ — см. правило двух копий в п.3.
...
ЖЁСТКОЕ ПРАВИЛО:
ПЕРЕД grep/read/task СНАЧАЛА query graph.json (кроме случаев «РАЗРЕШЕНО» ниже).
Нет graph.json → сначала graphify . --code-only.
...
3. Для логов и JSON — автоматическое сжатие через прокси (если `headroom proxy` запущен; иначе ручное).
